...
export default function SnakeRevealTop() {
    ...
    const { ref: ref6, rotate: rotate6 } = useRotateScroll({rotateDeg:60});
    ...
    const { ref: ref55, y: y55 } = useParallax({ speed: 0.16 });
    const { ref: ref66, y: y66 } = useParallax({ speed: 0.14 });
    const { ref: ref77, y: y77 } = useParallax({ speed: 0.1 });
    ...
    return (
        <motion.div
            ref={ref}
            style={{ y }}
            className="absolute top-50 left-0 w-full -z-10 h-full  min-w-full "
            initial={{ opacity: 0 }}
        ...
        >
            <svg
                ...
                className="w-full h-auto -z-10 opacity-60  overflow-y-visible overflow-x-clip "
            >
                <defs>
