"use client"
import { useParallaxx } from "@/hooks/paralllelx"
import { motion } from "framer-motion"


export function BgProp() {
    const { ref: chakraRef, y: chakraY, rotate: chakraRotate ,x:chakraX} =
        useParallaxx({ speed: 0.3, isRotate: true })
    const { ref: flowerRef, y: flowerY, rotate: flowerRotate,x:flowerX } =
        useParallaxx({ speed: 0.2, isRotate: true })
    const { ref: triangleRef, y: triangleY, rotate: triangleRotate,x:triangleX } =
        useParallaxx({ speed: 0.6, isRotate: true })
    const { ref: circle1Ref, y: circle1Y, rotate: circle1Rotate,x:circle1X } =
        useParallaxx({ speed: 0.2, isRotate: true })
    const { ref: circle2Ref, y: circle2Y, rotate: circle2Rotate,x:circle2X } =
        useParallaxx({ speed: 0.1, isRotate: true })

    return (
        <motion.div className="absolute --z-10 -left-32">
            <motion.svg
                width="539"
                height="536"
                viewBox="0 0 539 536"
                ...
                className="opacity-30 overflow-y-visible overflow-x-clip"
